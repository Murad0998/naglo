...
import random
from cfg import token, chanel, chanel_id
...
def start_markup():
    ...
    link_keyboard = types.InlineKeyboardButton(text="Подписаться", url=chanel)
    ...
    if data is None:
        ...
        if data1 is None:
            bot.send_message(message.chat.id,
                             f'{first_name}, анкета была заполнена не полностью. Пожалуйста, пройдите регистрацию заново')
            connect.close()
            restart(message)
        else:
...
def admin(message):
    if bot.get_chat_member(chat_id=chanel_id, user_id=message.chat.id).status == 'administrator' or 'creator':
        bot.send_message(message.chat.id, "Приветсвую, повелитель !", reply_markup=admin_markup())
    else:
        bot.send_message(message.chat.id, "У вас нет прав администратора")


def check3(call):
    if bot.get_chat_member(chat_id=chanel_id,
                           user_id=call.message.chat.id).status == 'member' or 'administrator' or 'creator':
        connect = sqlite3.connect('users.db')
        cursor = connect.cursor()
...
def novreg(call):
    if bot.get_chat_member(chat_id=chanel_id,
        ...
        bot.send_message(call.message.chat.id, 'Хорошо, сделаем тебе новую анкету!\n'
                         ...
                         reply_markup=start_markup6())  # СДЕЛАТЬ ПОЛНОСТЬЮ НОВОРЕГ КРОМЕ ID
    else:
        bot.send_message(call.message.chat.id, "Подпишитесь на канал!", reply_markup=proverka_markup())


def restart(message):
    if bot.get_chat_member(chat_id=chanel_id, user_id=message.chat.id).status == 'member':
...
def check(call):
    ...
    for i in status:
        if i == bot.get_chat_member(chat_id=chanel_id, user_id=call.message.chat.id).status:
            markup = types.InlineKeyboardMarkup(row_width=True)
...
def callback(call):
    ...
    if call.data == 'парни1':  # я апдейт
        ...
        if data is not None:
            # Изменение в таблице
            cursor.execute("UPDATE interest_id SET interest = ? WHERE id = ?",
                           ("Парни", call.message.chat.id))  # я апдейт
            connect.commit()
            kurs_update(call)
        else:
            # Запись в таблицу под рестарт
    ...
    if call.data == 'девушки1':  # я апдейт
        ...
        if data is not None:
            # Изменение в таблице
            cursor.execute("UPDATE interest_id SET interest = ? WHERE id = ?",
                           ("Девушки", call.message.chat.id))  # я апдейт
            connect.commit()
            kurs_update(call)
        else:
            # Запись в таблицу под рестарт
    ...
    if call.data == 'все равно1':
        ...
        if data is not None:
            # Изменение в таблице
            cursor.execute("UPDATE interest_id SET interest = ? WHERE id = ?",
                           ("Все равно", call.message.chat.id))  # я апдейт
            connect.commit()
            kurs_update(call)
        else:
            # Запись в таблицу под рестарт
        ...
        if 2 == 2:
            ...
            if data_text1 is not None:
                ...
                if 3 == 3:
                    ...
                    bot.send_message(call.message.chat.id, f"Начните общение прямо сейчас! -> @{user_name}",
                                     reply_markup=vzanketa1())
        connect.close()
...
def vzlike_ank(message, dizl=None):
    if bot.get_chat_member(chat_id=chanel_id,
                           user_id=message.chat.id).status == 'member' or 'administrator' or 'creator':
        try:
                ...
                try:
                    ...
                    if data_text is not None:
                        if vzlike1 is not None:
                            years, sex, name, kurs, interest, description = data_text
                            message_text = f"{name} {years} ({sex})\nКурс: {kurs}\nИнтересы: {interest}\nО себе: {description}"
                            bot.send_photo(message.chat.id, photo=photo_data, caption=message_text,
                                           reply_markup=grade_anketa1())
                except:
                    bot.send_message(message.chat.id,
                                     f'Анкет нет!\nОцените кого-то вы, возможно, оценка будет взаимной!',
                                     reply_markup=start_anketa())
        except:
...
def age(message):
    if bot.get_chat_member(chat_id=chanel_id,
                           user_id=message.chat.id).status == 'member' or 'administrator' or 'creator':
        if message.text:
            if (message.text.isalpha()) and (
                    len(message.text) < 17):  # ISALPHA ПРОВЕРЯЕТ ТОЛЬКО ИЗ БУКВ СОСТОИТ СТРОКА ИЛИ НЕТ
                connect = sqlite3.connect('users.db')
                cursor = connect.cursor()
                ...
                bug10 = bot.send_message(message.chat.id,
                                         'В имени не должны присутствовать цифры или посторонние символы кроме букв!\n'
...
def course(message):  # если тип в возрасте отправил не цифры! ебни проверку!!
    if bot.get_chat_member(chat_id=chanel_id,
                           user_id=message.chat.id).status == 'member' or 'administrator' or 'creator':
        if message.text:
            if (message.text.isdigit()) and (len(message.text) < 3) and (
                    len(message.text) > 1):  # КАК Я ПОНЯЛ ISDIGIT ПРОВЕРЯЕТ ТОЛЬКО ЦИФРЫ В СООБЩЕНИИ ИЛИ НЕТ
                connect = sqlite3.connect('users.db')
                cursor = connect.cursor()
                ...
                bug = bot.send_message(message.chat.id,
                                       'В сообщении присутствуют не только числа, либо вы слишком малы!\nОтправь только число')
        ...
        if message.text:
            if (message.text.isdigit()) and (len(message.text) < 2):
                ...
                if data is None:
                    ...
                    connect.commit()
                    connect.close()
                msg6 = bot.send_message(message.chat.id,
                                        'Расскажи немного о себе!\nЭто поможет лучше подобрать тебе компанию.')
                bot.register_next_step_handler(msg6, photos)
            else:
                bug1 = bot.send_message(message.chat.id,
                                        'В сообщении присутствуют не только числа, либо такого курса не существует!\n'
        ...
        if message.photo:  # проверка что отправили фото
            ...
            if file_info is not None:
                ...
                if data is None:
                    ...
                    cursor.execute("INSERT INTO photo_id (id, photo) VALUES (?, ?);",
    ...
    if bot.get_chat_member(chat_id=chanel_id,
        ...
        if data_photo is not None:  # проверка на наличие фото по айди
            ...
            if data_text is not None:
                ...
                bot.send_photo(call.message.chat.id, photo=photo_data, caption=message_text,
                               reply_markup=dlya_moya_anketa())
                connect.close()
    else:
        bot.send_message(call.message.chat.id, "Подпишитесь на канал!", reply_markup=proverka_markup())
...
def anketa_p(chat_id, result_list):
    if bot.get_chat_member(chat_id=chanel_id, user_id=chat_id).status == 'member' or 'administrator' or 'creator':
        ...
        result_list2 = ','.join(
            map(str, result_list))  # Делаем просто строку из id убрав список, для того, чтобы записать в бд
    ...
    if bot.get_chat_member(chat_id=chanel_id, user_id=message.chat.id).status == 'member':
        if message.text:
            if (message.text.isdigit()) and (len(message.text) < 2):
                ...
                if data is not None:
                    # Изменение в таблице
                    cursor.execute("UPDATE kurs_id SET kurs = ? WHERE id = ?",
                                   (message.text, message.chat.id))  # я апдейт
                    connect.commit()
                    connect.close()
                novmsg26 = bot.send_message(message.chat.id,
                                            'Расскажи немного о себе!\nЭто поможет лучше подобрать тебе компанию.')
                bot.register_next_step_handler(novmsg26, novphotos2)
            else:
                bug5 = bot.send_message(message.chat.id,
                                        'В сообщении присутствуют не только числа, либо такого курса не существует!'
                                        'Отправь только число или верный курс')
...
def novphotos2(message):
    if bot.get_chat_member(chat_id=chanel_id,
        ...
        if message.text:
            ...
            if data is not None:
                # Изменение в таблице
                cursor.execute("UPDATE description_id SET description = ? WHERE id = ?",
                               (message.text, message.chat.id))  # я апдейт
                connect.commit()
                connect.close()
    ...
    if bot.get_chat_member(chat_id=chanel_id,
        ...
        if message.text:
            if (message.text.isalpha()) and (len(message.text) < 15):
                ...
                if data is not None:
                    # Изменение в таблице
                    cursor.execute("UPDATE name_id SET name = ? WHERE id = ?",
                                   (message.text, message.chat.id))  # я апдейт
                    connect.commit()
                else:
    ...
    if bot.get_chat_member(chat_id=chanel_id,
        ...
        if message.text:
            if (message.text.isdigit()) and (len(message.text) < 3):
                ...
                if data is not None:
                    # Изменение в таблице
                    cursor.execute("UPDATE years_id SET years = ? WHERE id = ?",
                                   (message.text, message.chat.id))  # я апдейт
                    connect.commit()
                else:
                ...
                bug3 = bot.send_message(message.chat.id,
                                        'В сообщении присутствуют не только числа, либо ваш возраст слишком большой!\n'
        ...
        if message.text:
            if (message.text.isdigit()) and (len(message.text) < 2):
                ...
                if data is not None:
                    ...
                    connect.commit()
                novmsg6 = bot.send_message(message.chat.id,
                                           'Расскажи немного о себе!\nЭто поможет лучше подобрать тебе компанию.')
                bot.register_next_step_handler(novmsg6, novphotos)
            else:
                bug4 = bot.send_message(message.chat.id,
                                        'В сообщении присутствуют не только числа, либо такого курса не существует!\n'
                                        'Отправь только число или верный курс')
